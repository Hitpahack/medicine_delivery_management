...
export class AddDoctorComponent extends AdminBaseComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (this.doctorId && this.doctorId !== 0) {
      ...
      this.adminuserservice.getUserbyId(this.doctorId).subscribe((response) => {
        if (response?.isSuccess && response.data) {
          ...
          console.error("Failed to load doctor data", response);
        }
      });
  ...
  onSubmit() {
    if (this.doctorId && this.doctorId !== 0) {
      this.adminuserservice.edituser(this.editUserForm.value, this.doctorId).subscribe({
        next: (response) => {
          if (response.isSuccess) {
              Helper.ShowSuccess(response.message || 'doctor Updated successfully.');
      ...
      this.adminuserservice.add(dto).subscribe({
        next: (response) => {
          if (response.isSuccess) {
            Helper.ShowSuccess(response.message || 'doctor added successfully.');
            this.router.navigate(['/admin/doctor/list']);
          } else {
            console.error('API returned isSuccess: false');
            this.errorMessage = response.message || 'Failed to add doctor.';
            Helper.ShowError(this.errorMessage);  // Optional toast/popup
          }
        },
        error: (err) => {
